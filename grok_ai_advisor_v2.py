...
if page == "Portfolio Overview":
    ...
    if alerts:
        ...
        st.success("✅ All high-yield positions performing well.")

    if current_vix > 25:
        st.info("🌪️ High VIX — excellent environment. Hold or add.")
    elif current_vix < 16:
        st.info("📉 Low VIX — premiums shrinking. Consider trimming weak positions.")
    ...
    with st.form("sell_form"):
        ...
        if st.form_submit_button("Execute Sell + Rotate"):
            st.success(f"✅ Sold ${sell_amount:,.0f} of {sell_asset} and rotated to {rotate_to}. Adjust tracker manually if needed.")
