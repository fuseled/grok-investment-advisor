...
if page == "Portfolio Overview":
    ...
    fig = px.line(x=months, y=base_growth, markers=True, labels={"x": "Months Ahead", "y": "Projected Portfolio Value ($)"}, title="Portfolio Value Growth Over Next 12 Months (Base Case)")
    st.plotly_chart(fig, use_container_width=True)
    ...
    # ==================== HIGH-YIELD ONLY AI ADVISOR ====================
    st.subheader("High-Yield Slice Management")
    st.subheader("🤖 AI Sell / Rotate Advisor (High-Yield Only)")

    hy_df = st.session_state.high_yield_tracker.copy()
    hy_df["Current_Price"] = hy_df["Asset"].map(prices).fillna(1.0)
    ...
    for _, row in hy_df.iterrows():
        if row["Unrealized_PL_Pct"] < -25:
            alerts.append(f"**{row['Asset']}**: Heavy NAV decay ({row['Unrealized_PL_Pct']:.1f}%). Consider selling all and rotating.")
        elif row["Total_Return_Pct"] < avg_return - 15:
            alerts.append(f"**{row['Asset']}**: Underperforming slice ({row['Total_Return_Pct']:.1f}% vs avg {avg_return:.1f}%). Rotate recommended.")

    if alerts:
        st.warning("\n\n".join(alerts))
    else:
        st.success("✅ High-Yield slice looks healthy. No immediate sells recommended.")

    if current_vix > 25:
        st.info("🌪️ High VIX — excellent environment. Hold or add.")
    elif current_vix < 16:
        st.info("📉 Low VIX — premiums shrinking. Consider trimming weakest high-yield positions.")
    ...
    display_cols = ["Asset", "Cost_Basis", "Cum_Dividends", "Current_Value", "Unrealized_PL", "Unrealized_PL_Pct", "Total_Return_Pct", "Months_Held"]
    display_df = hy_df[display_cols].round(2)
    edited_hy = st.data_editor(display_df, use_container_width=True, hide_index=True, num_rows="fixed")

    # Sell & Rotate
    with st.form("sell_form"):
        st.write("**Sell & Rotate (High-Yield Only)**")
        sell_asset = st.selectbox("Asset to Sell", hy_df["Asset"].unique())
        sell_amount = st.number_input("Sell Amount ($)", value=500.0, step=100.0)
        rotate_to = st.selectbox("Rotate Proceeds Into", ["NVDY", "ULTY", "CHPY", "MRNY", "YMAX", "Cash"])
        if st.form_submit_button("Execute Sell + Rotate"):
            st.success(f"✅ Sold ${sell_amount:,.0f} of {sell_asset} and rotated to {rotate_to}.")
    ...
    with st.form("high_yield_form"):
        ...
        if st.form_submit_button("Add / Increase Position"):
            new_row = pd.DataFrame([{"Asset": hy_asset, "Cost_Basis": float(hy_amount), "Cum_Dividends": 0.0, "Purchase_Date": datetime.now().date()}])
    ...
    # ==================== CORE & QUALITY (Stable - No Sell Advisor) ====================
    st.subheader("Core Stable Specific Purchase")
    st.subheader("AI Advisor for Core Stable")
    st.write("**JEPI or JEPQ** — Strong, reliable monthly income. **Hold long-term.**")
    ...
    st.write("**SCHD / VIG** — Quality long-term holds.")
