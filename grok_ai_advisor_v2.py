...
# Aggressive slice for AI Analyst
...
if page == "📊 Portfolio Overview":
    # (All your existing overview code)
    ...
    # Sunburst + Category Summary (unchanged)
...
# (All other pages like Income Projections, Holding Details, Portfolio Combined remain unchanged)
    ...
    **Allocation Rule**:
    ...
    # AI Analyst on Reinvestment page too
    st.subheader("🔍 AI Analyst: High-Yield ETF Recommendation")
    if current_vix > 28:
        rec = "🚀 **ULTY or MRNY** — Highest premiums right now. Strong buy for the tactical slice."
    elif current_vix > 22:
...
st.caption(f"Last updated: {datetime.now().strftime('%B %d, %Y at %I:%M %p')}")import streamlit as st
import yfinance as yf
import pandas as pd
import plotly.express as px
from datetime import datetime, timedelta
